...
class ArcStiffness(HystereticForce):
    ...
    def gather_parameters_from_backbone(Q, XlamP_full, M, K, mode_select='nearest'):
        ...
        # target stiffness value (half of kt)
        target = 0.5 * kt
    
        # For each nonlinear DOF, find where k_nl crosses target and interpolate S from nlharmnorms
        for nl in range(Nnl):
            A = nlharmnorms[:, nl]   # amplitude-like measure, length Ncont
            Kvals = k_nl             # global scalar stiffness per continuation (length Ncont)
    
            # find first adjacent pair that brackets the target (assumes k_nl finite)
            diff = Kvals - target
            sign_changes = np.where(np.sign(diff[:-1]) * np.sign(diff[1:]) <= 0)[0]
    
            if sign_changes.size == 0:
                # no crossing found -> leave NaN
                S[nl] = np.nan
                B[nl] = np.nan
                continue
            ...
            i = sign_changes[0]
            A_i, A_ip1 = A[i], A[i+1]
            K_i, K_ip1 = Kvals[i], Kvals[i+1]
    
            if np.isclose(K_ip1, K_i):
                S[nl] = 0.5 * (A_i + A_ip1)
            else:
                frac = (target - K_i) / (K_ip1 - K_i)
                S[nl] = A_i + frac * (A_ip1 - A_i)
