...
def findDryParameters(E_patches, KT_patches0, KN_patches0, MU_patches0, ti_kt, tau_kt_patches, ti_mu, tau_mu_patches):
    '''
    ...
    KT_mod = lambda E, tau_kt: nlutils.first_order_exponential(1.0, ti_kt, tau_kt, E)
    KN_mod = lambda E, tau_kt: nlutils.first_order_exponential(1.0, ti_kt, tau_kt, E)
    MU_mod = lambda E, tau_mu: nlutils.first_order_exponential(1.0, ti_mu, tau_mu, E)
    ...
    for patch in range(Npatches):
        KT_patches[patch] = KT_patches0[patch] * KT_mod(E_patches[patch], tau_kt_patches[patch])
        KN_patches[patch] = KN_patches0[patch] * KN_mod(E_patches[patch], tau_kt_patches[patch])
        MU_patches[patch] = MU_patches0[patch] * MU_mod(E_patches[patch], tau_mu_patches[patch])
...
'''
Variable to choose amount of data to include in fitting
'''
...
mask = t_blocks_hours < data_hours
t_blocks_hours = t_blocks_hours[mask]
cumE = cumE[mask]
diss = diss[mask]
Kt_norm = Kt_norm[mask]
...
def return_E_dot_from_system(KT_patches0, KN_patches0, E_patches, unweathered, Aend):
    vib_sys = VibrationSystem(M, K)
    KT_patches, KN_patches, MU_patches = findDryParameters(E_patches, KT_patches0, KN_patches0, MU_patches0, ti_Kt, tau_Kt_per_patch, ti_mu, tau_mu_per_patch)
    ...
    if unweathered: #Unworn system energy loss calcaultion for elastic dry friction
        print("Unweathered calcaultion of Aend")
        patch_range = range(Nnl // 3)
    ...
    # --- Build interpolated harmonic vector at amplitude A_end ---
    A_levels = Uwxa_full_worn[:, -1]
    Uwxa_interp = np.zeros(Uwxa_full_worn.shape[1])
    
    Uwxa_interp = np.apply_along_axis(
        lambda col: np.interp(Aend, A_levels, col),
        axis=0,
        arr=Uwxa_full_worn
    )


    print(f"Amptliude of interp: {Uwxa_interp[-1]}")
    Edot_patches = np.zeros(Nnl // 3) #Npatches = Nnl // 3
    patch_range = range(Nnl // 3)
    for patch in patch_range:
        Edot = calculate_E_dot(Uwxa_interp, h, Nt, Uwxa_interp[-3], vib_sys.nonlinear_forces[patch], Aend)
...
integration_method = 'RK4' #euler or heun
...
slow_dts = np.array([7200, 3600, 1800, 900])
...
#%% Slow Time Step    
    for i in range(len(slow_times) - 1):
        ...
        if i == 0: 
            unweathered = True
        else: unweathered = False
        
        if integration_method == 'heun':
            #Predicted (Euler) Step
            
            print(f"======== Prediction ==========")
            
            Edot_patches_predicted = return_E_dot_from_system(KT_patches0,
                                                              KN_patches0, 
                                                              E_patches,
                                                              unweathered,
                                                              Aend)
            E_patches_predicted = E_patches + Edot_patches_predicted * dT
            
            print(f"======== Modified ==========")
            
            Edot_patches_modified = return_E_dot_from_system(KT_patches0, 
                                                              KN_patches0, 
                                                              E_patches_predicted,
                                                              False,
                                                              Aend)
            
            Edot_patches_corrected = 1/2 * (Edot_patches_predicted + 
                                            Edot_patches_modified)
            
            E_patches = E_patches + Edot_patches_corrected * dT
            
            E_patches_history[i+1, :] = E_patches
            E_dot_patches_history[:, i] = Edot_patches_corrected
            ...
            # k1
            k1 = return_E_dot_from_system(KT_patches0,
                                         KN_patches0,
                                         E_patches,
                                         unweathered,
                                         Aend)
            ...
            # k2
            E_tmp = E_patches + 0.5 * k1 * dT
            k2 = return_E_dot_from_system(KT_patches0,
                                         KN_patches0,
                                         E_tmp,
                                         unweathered,
                                         Aend)
            ...
            # k3
            E_tmp = E_patches + 0.5 * k2 * dT
            k3 = return_E_dot_from_system(KT_patches0,
                                         KN_patches0,
                                         E_tmp,
                                         unweathered,
                                         Aend)
            ...
            # k4
            E_tmp = E_patches + k3 * dT
            k4 = return_E_dot_from_system(KT_patches0,
                                         KN_patches0,
                                         E_tmp,
                                         unweathered,
                                         Aend)
    
            # Combine for RK4 update
            Edot_patches_corrected = (k1 + 2*k2 + 2*k3 + k4) / 6.0
            ...
            Edot_patches= return_E_dot_from_system(KT_patches0,
                                                              KN_patches0, 
                                                              E_patches,
                                                              unweathered,
                                                              Aend)
