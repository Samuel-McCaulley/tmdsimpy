...
Uwxa0[-1] = Astart
if not os.path.exists('./results/Uwxa_full.npy'):
    ...
    np.save("./results/Xpre_RC.npy", Xpre)
Uwxa_full = np.load('./results/Uwxa_full.npy')
#%% Calculation of Initial Jenkins Parameter Functions wrt Normal Force
# (D = 0 Loading)

Xpre_RC = np.load('./results/Xpre_RC.npy')

ElasticModStar = ElasticMod/(2*(1-PoissonRatio**2))
ShearModStar = ElasticMod/4/(1+PoissonRatio)/(2-PoissonRatio)

KT_patches = np.zeros(Nnl)
KN_patches = np.zeros(Nnl)

patch_range = range(Nnl)
...
def findDryParameters(E_patches, KT_patches0, KN_patches0):
    ...
    #Placeholder KT, KN
    KT_mod = lambda E: np.exp(-E/1)
    KN_mod = lambda E: 1 - 0.5 * np.exp(-E/2)
...
# for patch in patch_range: #For each patch, I DONT THINK THIS WORKS RIGHT NOW
#     uxyn_patch = (QL @ Xpre_RC)[3*patch:3*patch+3]
#     uxyn_0 = np.zeros_like(uxyn_patch)
#     un_patch = uxyn_patch[-1]
#     patch_area = np.sum(Tm[:, patch])
#     asperity_number = patch_area * area_density
    
#     delta = np.array(un_patch - meso_gap_quads[patch] - gaps) # I am not sure if this is right
#     delta[delta <= 0] = 0
    
#     # F_N = area * 4/3 * ElasticModStar * np.sqrt(Radius) * np.sum(np.power(contact_displacement, 1.5) @ gap_weights)
#     # k_N = asperity_number * 2 * ElasticModStar * np.sqrt(Radius) * np.sum(np.power(contact_displacement, 0.5) @ gap_weights)
#     # k_T = asperity_number * 8 * ShearModStar * np.sqrt(Radius) * np.sum(np.power(contact_displacement, 0.5) @ gap_weights)
    
#     # Plasticity Parameters
#     C = 1.295*np.exp(0.736*PoissonRatio);
    
#     # displacement of one sphere against a rigid flat to cause yielding.
#     delta_y = 2* (np.pi*C*YieldStress/(2*(2*ElasticModStar)))**2*(2*Radius); 
    
    
#     deltam = np.zeros_like(delta)
#     Fm = np.zeros_like(delta)
#     fn_asperity, a, deltabar, Rebar = _asperity_functions._normal_asperity_general(delta, deltam, Fm, 
#                                  Radius, PoissonRatio, ElasticModStar, ElasticMod, TangentMod, delta_y, YieldStress)
    
#     fxy_curr = _asperity_functions._tangential_asperity(uxyn_patch[:2], uxyn_0[:2], np.zeros((fn_asperity.shape[0], 2)), 
#                                          fn_asperity, a, ShearModStar, mu)
    
#     kt_asperity =  8 * ShearModStar * a
#     kn_asperity = 2 * ElasticModStar * a
    
#     KT = kt_asperity @ gap_weights * patch_area
#     KN = kn_asperity @ gap_weights * patch_area
...
# uxyn_est = sio.loadmat('data/TMD_smallbrb_upxyn.mat')['upxyn']
...
KT_patches = kt * patch_areas
KN_patches= kn * patch_areas



    
KT_patches0, KN_patches0 = KT_patches, KN_patches
slow_dts = np.array([3600, 1800])#, 900, 450, 225])
for dt_idx in range(len(slow_dts)):
    slow_times = np.array(range(0,10800+1,slow_dts[dt_idx]))
    
    Amplitude = 10**-0.5
    la = -0.5
    Aend = -0.5
    
    Uwxa_previous = np.zeros(Ndof * Nhc + 3)
    
    #Start time loop 
    E_patches = np.zeros(Nnl)
    E_patches_history = np.zeros((len(slow_times), Nnl))
    E_patches_history[0, :] = E_patches #E_patches is zero obviously
    for i in range(len(slow_times) - 1):
        print('')
        print(f"======== TIME: {slow_times[i]} ==========")
        print('')
        elastic_dry_vib_sys = VibrationSystem(system_matrices['M'], system_matrices['K'], ab=damp_ab)
        #elastic_dry_vib_sys = VibrationSystem(M, K, ab=damp_ab)
        ...
        for patch in patch_range:
            ...
            Ls = (QL[patch*3:(patch*3+3), :])
            Lf = (LTT[:, patch*3:(patch*3+3)])
            
            #Ls = Q[patch*3:(patch*3+3), :]
            #Lf = Ls.T
            
            patch_nlforce = ElasticDryFriction3D(Ls, Lf, KT_patches[patch], KN_patches[patch], mu)
            elastic_dry_vib_sys.add_nl_force(patch_nlforce)
            
        ##%% Prestress Analysis
        elastic_dry_vib_sys.set_prestress_mu()
    
        #Fv = system_matrices['Fv'][:, 0] #Fine
    
        # Get an estimate of the stiffness at a contact
        
        # t, dtduxyn = ref_nlforce.force(uxyn_est.reshape(-1))
        ...
        # function to solve
        pre_fun = lambda U, calc_grad=True : elastic_dry_vib_sys.static_res(U, Fv*prestress)
        ...
        # Update history variables after static so sliders reset
        # If you do not do this, then the residual traction field will be different.
        elastic_dry_vib_sys.update_force_history(Xpre)
    
        # Use the prestress solution as the intial slider positions for AFT as well
        # This influences residual tractions and may slightly change the results of
        # the simulation.
        elastic_dry_vib_sys.set_aft_initialize(Xpre)
    
        # Reset to real friction coefficient after updating frictionless slider
        # positions
        # This is needed so that the friction coefficient is used in EPMC 
        # (rather than 0 tangential forces)
        elastic_dry_vib_sys.reset_real_mu()
        
        
        
        ##%% Build EPMC Guess from Prestress Aanalysis
        
        Rpre, dRpredX = elastic_dry_vib_sys.static_res(Xpre, Fv*prestress)
        ...
        elastic_dry_vib_sys.set_new_C(C=pre_ab[0]*elastic_dry_vib_sys.M + pre_ab[1]*Kpre)
        ...
        epmc_fun = lambda Uwxa, calc_grad=True : elastic_dry_vib_sys.epmc_res(Uwxa, Fl, h, Nt=Nt, 
                                                                  calc_grad=calc_grad)
        ...
        Edot_patches = np.zeros(Nnl)
        for patch in range(Nnl):
            Edot = calculate_E_dot(Uwxa_full_worn[-1, :], h, Nt, Uwxa_full_worn[-1, -3], elastic_dry_vib_sys.nonlinear_forces[patch])
    ...
    plt.plot(slow_times, E_patches_average)
