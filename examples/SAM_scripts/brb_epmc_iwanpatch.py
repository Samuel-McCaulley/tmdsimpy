...
iwan_parameters = [10**5.530289365884571, 10**15.859238733093676, -0.999870602783605, 10**-5.746331062210725, 10**16.888652840667923] #best freq
...
Npatches = len(patch_areas)

for i in range(Npatches):
    Q_patch = Q[3*i:3*i+3, :]
    T_patch = T[:, 3*i:3*i+3]
    
    Fs = iwan_parameters[0] * patch_areas[i]
    Kt = iwan_parameters[1] * patch_areas[i]
    Chi = iwan_parameters[2]
    Bt = iwan_parameters[3]
    Kn = iwan_parameters[4] * patch_areas[i]
    
    patch_force = IwanPatch(Q_patch, T_patch, Kt, Fs, Chi, Bt, Kn)
